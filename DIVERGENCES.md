...
| D41 | md FRONT MATTER: at ABSOLUTE input start only, `---`+eol then `end_string "---"` (unclosed → normal parse; closer's line remainder re-enters the document); body `many1(key: value)` with consume:All — a non-kv line makes the WHOLE body yield ZERO directives while still consumed; key may contain spaces, `:` sep + MSPACE, raw value. lsdoc: root-only prefix parse in `parse()` itself (structurally unreachable from quote/list reparses), spans offset, scan_work-charged. Pins: fm1-fm10 + r6-r9 + m1-m6 in `notes/d41-d42-oracle-pins.md` | **FIXED** | `5f036e4` |
| D42 | md footnote-def rules (shared `footnote.ml` `footnote_definition`): name bans MSPACE; body = many1 of [MSPACE-skip + first byte ∉ {-,*,#,[,eol} + ≥1 more byte] (so ≥2 bytes/line, 1-byte/empty/`#`-led bodies → paragraph), MULTI-LINE fold joined by \n (per-line leading MSPACE dropped), wrapper consumes trailing eol RUN (D42b — the org side already did; md sites lacked it, found by MY verification probe, pre-existing via stash-check); BOTH md dispatch sites (top-level + dash-bullet title-lookahead split) share one folding routine. Pins fd1-fd10 + r1-r5 + e1-e7 + m7-m12 | **FIXED** | `5f036e4` |
| D43 | md inline backslash/backtick residue (= the ENTIRE extended-fuzz md floor ~65; the D39 leftover) — TWO pinned 1.5.7-ARTIFACT mechanisms (checkout ~1.5.9 does NOT produce them; oracle wins, same category as D19/D30): (A) SYNTHETIC triple-marker emphasis (`***…***`/`___…___`) applies ONE backslash-consumption layer to the plain body child (`***\`***` → Bold[plain "`"]; `\\`+bt → `\`+bt; guards: all non-synthetic markers/depths already match via markdown_plain_text); (B) md link-label first-stage scanner: a LONE backtick whose md_code fails is Plain("`") — lsdoc manufactures a Code node there (`[\\`](u)`: mldoc label plain "\`", lsdoc code "\"); the existing failed-reparse one-layer unescape explains the rest. Characterized by 31 isolated-oracle probes (`harness/d43-probe.json`, 9 DIFF / 22 guards; full pins `notes/d43-oracle-pins.md`); transcription audit `notes/inline-residue-transcription.md`. TRACED CAUSE of (B): `try_code_span` was invoked at offset 0 of every label plain chunk without checking the byte is a backtick, while `lexer::code_span` assumes caller-on-backtick — one guard line fixes it; (A) = the synthetic path now runs the same is_ascii_punctuation one-layer unescape as markdown_plain_text, spans via make_plain + span_map. Result: **extended-vocab fuzz floor 0/0 BOTH formats, 4 seeds** | **FIXED** | `818afb4` |
| D44 | UMBRELLA (Martin-requested enumerations 2a+2b, Jul 3): construct-SEAM cross-product (247,584 cases: all line-vocab pairs+triples both formats + CRLF pass) + small-string exhaustion (542,904 cases: len 1-5 over 12-byte structural alphabets) → **1,088 isolated-CONFIRMED classes ≈ ~10 MECHANISMS** (reduction: `notes/d44-mechanism-reduction.md`; findings `harness/enum-{seams,small}-findings.json`; enumerators `harness/enum-{seams,small}.mjs` + `lib/enum-runner.mjs`). Top mechanisms: md `\|-\|` table separator/header semantics (~727 classes); `\begin{x}..\end{x}` TAIL dispatch (~30 classes, EVERY following construct, both formats); stray-`:END:` drawer formation (~111 classes, md drawer + org example twins); md `---`+def-list interaction (~41); org directive-after-empty-headline (~39); md `>`-quote children w/ following construct (~52); md `>>` prefixes (~22); md `<!--`/`-->` Comment shapes (~17 — the D39 "refuted" md-comment prediction was INPUT-SPECIFIC, bodyless `<!--\n-->` IS mldoc Comment); md `# >*` heading-inline (~6, 165 cases); org list-FF residue + properties-after-blank + assorted singletons. Triage COMPLETE (Jul 3 evening): transcription audit `notes/d44-transcription.md` (all 10 mechanisms, mldoc+lsdoc citations) + oracle confirmation `harness/d44-probe.json` (61 probes: **48 REAL / 13 guards**, full pins `notes/d44-oracle-pins.md`). Audit correction: the table mechanism is NARROWER than transcribed (standalone `|-|` + header-then-separator MATCH; only separator-then-nontable + double-separator diverge). Fix plan = 8 grouped rounds (A table core + quote-table children; B latex-env tail both formats; C drawer/property grouping incl. stray-`:END:` + properties-after-blank + empty-PROPERTIES fold; D heading title-lookahead md+org; E md def-lists incl. HR-term + trailing-ws; F md `>>` quote prefixes; G md comments; H org list-FF sibling nesting). ALL 8 ROUNDS FIXED (Jul 3): A tables `4a2a2e0` (+quote-path TBLFM round-2), B latex-tail `22fac3c`, C drawer/property `707fee6`, D heading-lookahead `3f2d979` (+hiccup round-2), E def-lists `2f14838`, F+G quotes+comments `b4a5d91` (+org-twin round-2), H list sibling-across-`two_eols` BOTH formats `a235850` (boundary-aware `nest_items_with_boundaries`; global `nest_items` untouched). Final acceptance: d44-probe 61/61 + ALL per-round extra/adversarial/verify batches green (vdiff_iso), full gate 1363/1363, fuzz seeds 99/31337/271828 both formats 0/0, cargo suites green; full enum re-runs post-fix (see below/notes). Regression material: `harness/d44*-probe.json` + `harness/d44*-verify-my.json`. POST-FIX full enum re-runs (Jul 3 late): seams 247,584 → **4** confirmed classes (was 1,044), small 542,904 → **7** (was 44), clearindents2 427,814 → **0**; residue filed as D45 | **FIXED** | `4a2a2e0`..`a235850` |
| D45 | ENUM RESIDUE (post-D44 re-run, Jul 3): **11 isolated-confirmed classes ≈ ~5 mechanisms**, all md; 17 pinned reps in `harness/d45-residual-probe.json` (ALL vdiff_iso-REAL), findings `harness/enum-{seams,small}-findings.json`. Mechanisms: (1) HR-as-def-list-term OUTSIDE plain top-level — inside `>`-quote reparse (`> q\n---\n: def x`, `***` twin) and after empty heading (`## \n---\n: def x`), + latex-env-vs-def-list in quote (`> q\n\begin{x}\n: def x`); (2) empty-heading + md-comment lookahead (`## \n<!--\n-->`: mldoc heading+para+comment, lsdoc heading+comment — heading title-lookahead eats a line); (3) md inline emphasis-with-backslash family (`*\*\`, `-*\*\`, `*\*#\`, leading-`\n` variants: mldoc Italic emphasis containing `\`, lsdoc plain — D43-adjacent, the try_code_span-era residue at the emphasis layer); (4) `>>` quote + blank + following construct (`>>\n\n*`: content grouping across the blank); (5) NESTED `>>`-quote recursion (`>>>>`, `>> >>`, `\n>>>>`: mldoc quote(quote(para(break))), lsdoc single quote w/ literal `>>` text — Round F fixed only the empty-first-line branch, not recursive quote nesting) | **OPEN** | — |
| D46 | **REAL-GRAPH divergences (Martin's graph-check run on his actual Logseq graph, Jul 3)** — 4 findings → 3 mechanisms, all md, pins `harness/d46-realgraph-probe.json` (g1-g4) + `harness/d46-pins-probe.json` (40 probes: 20 real / 20 guards; full outputs `subagent-tasks/notes/d46-vdiff-full.txt`): (A) dash-bullet title lookahead is a PARTIAL replica of heading0.ml `title_aux_p` — Drawer/Hr-subset/Table/Latex_env/math/raw-html/Footnote missing on the `- ` path while the `#` path has the full `md_heading_split_opener` (the `- :LOGBOOK:\n  CLOCK:…\n  :END:` shape = bullet + drawer("logbook") in mldoc; also `- :PROPERTIES:` → properties; marker preserved on split, a14); (B) task-marker boundary = space OR END-OF-INPUT (heading0.ml peek_char None ⇒ marker): md `- DONE`⟨EOF⟩ must get marker (lsdoc requires trailing space), org has the OPPOSITE bug (`* DONE\n` gets a marker in lsdoc; mldoc rejects before eol); (C) md table `separated_line` (table.ml:30-36) = spaces `\|` take_while1{-,+,\|,sp,:} eol — NO closing-pipe requirement, all-space run qualifies (`\t  \| ` starts a header:null table; lsdoc demands ends-with-`\|`). Spec `subagent-tasks/d46-realgraph-spec.md` | **OPEN (spec-check in flight)** | — |
