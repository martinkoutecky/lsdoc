...
| D40 | SUB/FF whitespace-SET class at BLOCK OPENERS (one mechanism, ~18 sites, BOTH formats): mldoc `spaces`/`ws` skip `{' ','\t','\x1a','\f'}` (`parsers.ml:8`; `is_tab_or_space` is aliased to `is_space`) at every `between_eols`-wrapped opener + list indent/marker-sep + heading post-marker (`whitespace_chars` incl. FF), while lsdoc gates on space/tab `leading_ws` or Rust-trim. 29 oracle-CONFIRMED probes (`harness/d39-transcription-probe.json`): md ATX/dash-bullet/list/table/HR/quote/callout/fence/latex-env/dmath/raw-html/hiccup/directive/`#+X:`-property, org verbatim/drawer/headline(post-star FF + trailing split)/list(indent + FF continuation)/table/HR/footnote-def/comment. Fix = ONE cross-opener round: route the opener gates through a shared mldoc-ws helper (the exact `org_space`/`md_property_space` pattern already proven at 3 sites) | **OPEN** | — |
| D41 | md FRONT MATTER not implemented: at absolute parser start `---\n…\n---` parses as `key: value` directives in mldoc (`markdown_front_matter.ml:24-30`, no leading-ws tolerance, tried before the whole document `many`); lsdoc parses the fences as HR. Oracle-confirmed (`d39p-md-frontmatter`) on default config | **OPEN** | — |
| D42 | md footnote-def body rule: `[^1]: a` — mldoc's shared `footnote_definition` requires each body line to have ≥2 bytes after the leading-ws skip (`footnote.ml:57-73` via `line` after one non-special byte), so a 1-byte body fails → paragraph; lsdoc's `footnote_def` accepts any nonempty body. Oracle-confirmed (`d39p-md-fndef-short`). Full body/continuation rules need the transcription treatment when fixing | **OPEN** | — |
