...
| D39 | UMBRELLA (triage pending): the extended fuzz vocabulary (frame/ws/`\f` tokens, `1cd4927`) surfaces further divergences beyond the enumeration's frame-body scope — md seed 99: 65 block; org seed 99: 64 refs + 354 blocks. Raw findings captured in `harness/d39-fuzz-findings.txt`; shapes look construct-SEAM-flavored (`\f* #+NAME:`, drawers after `\f`, lists meeting `#+END_` lines, `$$` frames). Each mechanism gets the transcription treatment (read the whole mldoc function first), then a D-entry | **OPEN** | — |
